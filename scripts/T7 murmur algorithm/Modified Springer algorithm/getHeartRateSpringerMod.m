...
function [heartRate, systolicTimeInterval,acf,info] = getHeartRateSpringerMod(x, fs, varargin)
...
p = inputParser;
addOptional(p, 'figures', figures);
addOptional(p, 't_HRsearch', t_HRsearch);
addOptional(p, 'thrHR', thrHR);
addOptional(p, 'thrMult', thrMult);
parse(p,varargin{:});

figures    = p.Results.figures;
t_HRsearch = p.Results.t_HRsearch;
thrHR   = p.Results.thrHR;
thrMult = p.Results.thrMult;
%% Get heatrate:
% From Schmidt:
...
min_index = floor(t_HRsearch(1)*fs);
max_index = t_HRsearch(2)*fs;
