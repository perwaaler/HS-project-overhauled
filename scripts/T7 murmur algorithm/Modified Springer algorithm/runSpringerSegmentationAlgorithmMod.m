...
function [assigned_states, info] = runSpringerSegmentationAlgorithmMod...
                                 (audio_data, fs, HMMpar, heartPar, figures)
% Runs MY VERSION of springers segmentation algorithm, for one recording
% audio_data. Fs is the sampling frequency of the signal, and Fs=Fs0/Nds if
% the signal has been downsampled. If the parameter heartPar is not
% provided, then it estimates it using my modified version of
% getHeartRateSchmidt.

% B_matrix  = HMMpar.Bmatrix;
% pi_vector = HMMpar.piVector;
% total_observation_distribution = HMMpar.totObsDist;
%% Preliminary
if nargin==3
    figures  = false;
    heartPar = [];
...
% note: if you want algorithm to compute heart parameters AND plot, the
% insert heartPar as [].

%% Get PCG Features:

[PCG_Features, featuresFs] = getSpringerPCGFeaturesMod(audio_data, fs);

%% Get PCG heart rate

% ¤¤¤ MODIFICATION ¤¤¤
if isempty(heartPar)
    [heartPar.rate, heartPar.sysDuration, acf, info] =...
                    getHeartRateSpringerMod(audio_data, fs);
else
    acf = nan;
end

[~,~,qt] = viterbiDecodePCG_SpringerMod(PCG_Features, HMMpar, heartPar, featuresFs); %#ok<*ASGLU>
