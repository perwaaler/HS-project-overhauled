...
if plotTot
    AUC = getAUCandPlotROC(activations,Ytrue,'plot',plotTot)
    title(sprintf('AUC=%g%%',round(AUC.whole,3)*100))
end
compactLinModelPresentation(glm)
% %% investigate biggest AS miss predictions:
% [m,J] = maxk(squaredLoss(activations,HSdata.ASgrade(JvalTot)),4)
% HSdata.ASgrade(JvalTot(J))
% close all
% quickScaleogram(HSdata.id(JvalTot(J(4))),1)
