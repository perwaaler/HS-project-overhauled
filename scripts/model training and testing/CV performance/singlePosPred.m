...
for i=1:Nsplits
    ...
    for aa=1:4
        ...
        activ = CVresults.train.activations{i,aa};
        [AUC,X,Y,T] = getAUCandPlotROC(activ, Y_target, 'plot', plotTrain)
        ...
        [AUC,X,Y] = getAUCandPlotROC(activ, Y_target, 'plot', plotValCV)
        ...
        Ytrue{i,aa}       = Y_target;
        corrPredCell{i,aa} = (activ>=u0)==(Y_target);
...
for aa=1:4
    ...
    AUC = getAUCandPlotROC(activ,Y_target,'plot',true)              
end
end

% *** plot ROC-curve for predictions, all positions combined ***
Y_target = cell2mat(reshape(Ytrue,[Nsplits*4,1]));
...
% figure
AUC = getAUCandPlotROC(activ,Y_target,'plot',plotValTotTot)
                            
predAccuracyTot = balancedPerfEstUsingCVoutput(pred,Y_target,1);
% title(sprintf('AUC=%g',AUC*100))
legend('murmur-cutoff=1 (AUC=0.940)', 'murmur-cutoff=2 (AUC=0.969)')
...
for aa=1:4
    ...
    activ = cell2mat(CVresults.val.activations(:,aa));
    if targetType=="murmur"
        Y_target = HSdata.(sprintf('murGrade%g',aa))(J)>=classThr;
    else
        Y_target = HSdata.(targetVar)(J)>=classThr;
    end
    if plotValTot
    ...
    AUC = getAUCandPlotROC(activ,Y_target,'plot',plotValTot)
    ...
    predMetrics = balancedPerfEstUsingCVoutput(pred,Y_target,1);
    % store in structure:
...
%% ****** End of Main Part of Script ******

%% estimate PERFORMANCE of MAX-MURMUR-PREDICTION algorithm
% ¤¤ SET NUMBER OF CV-SPLITS ¤¤
Nsplits = 8;
...
for i=1:Nsplits
    ...
    if estimatePredictiveModel && targetType=="AS"
        plotVal = true;
        [activ,u0,Y_target,AUC] = get_ASactivations(ActMat,HSdata,...
    ...
    [~,X,Y,T,~] = getAUCandPlotROC(activ,Y_target,'plot',plotTrain)
    ...
    [AUC,X,Y] = getAUCandPlotROC(activ,Y_target,'plot',plotVal); %#ok<*ASGLU>
    ...
    ac(i) = mean((activ>=u0)==Y_target);
    sn(i)  = condProb(activ>=u0,Y_target);
    sp(i)  = condProb(activ<u0,~Y_target);
end
...
if plotValTot
    hold on
    getAUCandPlotROC(activations,YtargetTot,'plot',plotValTot)            
end
...
if plotTot
    figure
    getAUCandPlotROC(activations,Ytrue,'plot',plotTot)
end
