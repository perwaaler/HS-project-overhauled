function jointActivCell = getJointActivationCells(CVactivationArrays)
% CVactivationArrays contains two 8-by-4 cell arrays, one for the validation
% sets and one for the training sets, and is assumed to be the output of
% the function createJointValAndTrain, so that training predictions have
% been shifted to validation sets for prediction-padding. Outputs two 1 by
% 8 cell arrays, one for validation-sets and one for training-sets, each of
% which contains indeces and max-activations for the corresponding set.

%% example
% CVactivationArrays = new;
% get range
%%
Nsplits = height(CVactivationArrays.trainMat);
...
for k=1:2
    if k==1
        CVcellJoint = CVactivationArrays.valMat;
    else
        CVcellJoint = CVactivationArrays.trainMat;
    end
